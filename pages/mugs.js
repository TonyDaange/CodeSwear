import Link from "next/link";
import React from "react";

const Mugs = () => {
  return (
    <div>
      <section className="text-gray-600 body-font text-2xl lg:mt-35 xl:mt-25 mt-42">
        <div className="container px-4 py-20 mx-auto">
          <div className="flex flex-wrap justify-center ">
            <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-[0_4px_8px_0_rgba(0,0,0,0.2)] hover:shadow-[0_10px_20px_0_rgba(0,0,0,0.2)] transition-shadow duration-800] lg:m-5">
              <Link href="/product/wear-the-code-mug-dark-blue-with-spoon">
                <a className="block relative h-auto rounded overflow-hidden">
                  <img
                    alt="ecommerce"
                    className="w-auto h-[357px] lg:h-[150px] block m-auto lg:mx-0"
                    src="https://m.media-amazon.com/images/I/61F5++FEwjL._SL1500_.jpg"
                  />
                </a>
                <div className="mt-4 text-center lg:text-left">
                  <h3 className="text-gray-500 text-lg tracking-widest title-font mb-1">
                    Mugs
                  </h3>
                  <h2 className="text-gray-900 title-font text-2xl font-medium">
                    Wear the Code
                  </h2>
                  <p className="mt-1 text-gray-500">₹599.00</p>
                  {/* <p className="mt-1">S, M, L, XL, XXL</p> */}
                </div>
              </Link>
            </div>

            <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-[0_4px_8px_0_rgba(0,0,0,0.2)] hover:shadow-[0_10px_20px_0_rgba(0,0,0,0.2)] transition-shadow duration-800] lg:mx-5 lg:my-5">
              <Link href="/product/wear-the-code-mug-pink-with-lid">
                <a className="block relative h-auto rounded overflow-hidden">
                  <img
                    alt="ecommerce"
                    className="w-auto h-[357px] lg:h-[150px] block m-auto lg:mx-0"
                    src="https://m.media-amazon.com/images/I/51HuxJ+gA3L._SX679_.jpg"
                  />
                </a>
                <div className="mt-4 text-center lg:text-left">
                  <h3 className="text-gray-500 text-lg tracking-widest title-font mb-1">
                    Mugs
                  </h3>
                  <h2 className="text-gray-900 title-font text-2xl font-medium">
                    Wear the Code
                  </h2>
                  <p className="mt-1 text-gray-500">₹599.00</p>
                  {/* <p className="mt-1">S, M, L, XL, XXL</p> */}
                </div>
              </Link>
            </div>

            <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-[0_4px_8px_0_rgba(0,0,0,0.2)] hover:shadow-[0_10px_20px_0_rgba(0,0,0,0.2)] transition-shadow duration-800] lg:mx-5 lg:my-5">
              <Link href="/product/wear-the-code-mug-black-with-lid">
                <a className="block relative h-auto rounded overflow-hidden">
                  <img
                    alt="ecommerce"
                    className="w-auto h-[357px] lg:h-[150px] block m-auto lg:mx-0"
                    src="https://m.media-amazon.com/images/I/51aUrljzaEL._SX679_.jpg"
                  />
                </a>
                <div className="mt-4 text-center lg:text-left">
                  <h3 className="text-gray-500 text-lg tracking-widest title-font mb-1">
                    Mugs
                  </h3>
                  <h2 className="text-gray-900 title-font text-2xl font-medium">
                    Wear the Code
                  </h2>
                  <p className="mt-1 text-gray-500">₹599.00</p>
                  {/* <p className="mt-1">S, M, L, XL, XXL</p> */}
                </div>
              </Link>
            </div>

            <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-[0_4px_8px_0_rgba(0,0,0,0.2)] hover:shadow-[0_10px_20px_0_rgba(0,0,0,0.2)] transition-shadow duration-800] lg:mx-5 lg:my-5">
              <Link href="/product/wear-the-code-mug-pink-with-spoon">
                <a className="block relative h-auto rounded overflow-hidden">
                  <img
                    alt="ecommerce"
                    className="w-auto h-[357px] lg:h-[150px] block m-auto lg:mx-0"
                    src="https://m.media-amazon.com/images/I/61EYDfXFtgL._SX679_.jpg"
                  />
                </a>
                <div className="mt-4 text-center lg:text-left">
                  <h3 className="text-gray-500 text-lg tracking-widest title-font mb-1">
                    Mugs
                  </h3>
                  <h2 className="text-gray-900 title-font text-2xl font-medium">
                    Wear the Code
                  </h2>
                  <p className="mt-1 text-gray-500">₹599.00</p>
                  {/* <p className="mt-1">S, M, L, XL, XXL</p> */}
                </div>
              </Link>
            </div>

            <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-[0_4px_8px_0_rgba(0,0,0,0.2)] hover:shadow-[0_10px_20px_0_rgba(0,0,0,0.2)] transition-shadow duration-800] lg:mx-5 lg:my-5">
              <Link href="/product/wear-the-code-mug-dark-blue-with-crown-and-spoon">
                <a className="block relative h-auto rounded overflow-hidden">
                  <img
                    alt="ecommerce"
                    className="w-auto h-[357px] lg:h-[150px] block m-auto lg:mx-0"
                    src="https://m.media-amazon.com/images/I/71XEu6WHEQL._SX679_.jpg"
                  />
                </a>
                <div className="mt-4 text-center lg:text-left">
                  <h3 className="text-gray-500 text-lg tracking-widest title-font mb-1">
                    Mugs
                  </h3>
                  <h2 className="text-gray-900 title-font text-2xl font-medium">
                    Wear the Code
                  </h2>
                  <p className="mt-1 text-gray-500">₹599.00</p>
                  {/* <p className="mt-1">S, M, L, XL, XXL</p> */}
                </div>
              </Link>
            </div>

            <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-[0_4px_8px_0_rgba(0,0,0,0.2)] hover:shadow-[0_10px_20px_0_rgba(0,0,0,0.2)] transition-shadow duration-800] lg:mx-5 lg:my-5">
              <Link href="/product/wear-the-code-mug-pink-with-crown-and-spoon">
                <a className="block relative h-auto rounded overflow-hidden">
                  <img
                    alt="ecommerce"
                    className="w-auto h-[357px] lg:h-[150px] block m-auto lg:mx-0"
                    src="https://m.media-amazon.com/images/I/61RQnGwSHNL._SX679_.jpg"
                  />
                </a>
                <div className="mt-4 text-center lg:text-left">
                  <h3 className="text-gray-500 text-lg tracking-widest title-font mb-1">
                    Mugs
                  </h3>
                  <h2 className="text-gray-900 title-font text-2xl font-medium">
                    Wear the Code
                  </h2>
                  <p className="mt-1 text-gray-500">₹599.00</p>
                  {/* <p className="mt-1">S, M, L, XL, XXL</p> */}
                </div>
              </Link>
            </div>

            <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-[0_4px_8px_0_rgba(0,0,0,0.2)] hover:shadow-[0_10px_20px_0_rgba(0,0,0,0.2)] transition-shadow duration-800] lg:mx-5 lg:my-5">
              <Link href="/product/wear-the-code-mug-cyan-with-crown-and-spoon">
                <a className="block relative h-auto rounded overflow-hidden">
                  <img
                    alt="ecommerce"
                    className="w-auto h-[357px] lg:h-[150px] block m-auto lg:mx-0"
                    src="https://m.media-amazon.com/images/I/71qCGFl3CFL._SX679_.jpg"
                  />
                </a>
                <div className="mt-4 text-center lg:text-left">
                  <h3 className="text-gray-500 text-lg tracking-widest title-font mb-1">
                    Mugs
                  </h3>
                  <h2 className="text-gray-900 title-font text-2xl font-medium">
                    Wear the Code
                  </h2>
                  <p className="mt-1 text-gray-500">₹599.00</p>
                  {/* <p className="mt-1">S, M, L, XL, XXL</p> */}
                </div>
              </Link>
            </div>

            <div className="lg:w-1/5 md:w-1/2 p-4 w-full shadow-[0_4px_8px_0_rgba(0,0,0,0.2)] hover:shadow-[0_10px_20px_0_rgba(0,0,0,0.2)] transition-shadow duration-800] lg:mx-5 lg:my-5">
              <Link href="/product/wear-the-code-mug-black-with-crown-and-spoon">
                <a className="block relative rounded overflow-hidden">
                  <img
                    alt="ecommerce"
                    className="w-auto h-[357px] lg:h-[150px] block m-auto lg:mx-0"
                    src="https://m.media-amazon.com/images/I/51uPoDVLodL._SX679_.jpg"
                  />
                </a>
                <div className="mt-4 text-center lg:text-left">
                  <h3 className="text-gray-500 text-lg tracking-widest title-font mb-1">
                    Mugs
                  </h3>
                  <h2 className="text-gray-900 title-font text-2xl font-medium">
                    Wear the Code
                    <p className="mt-1 text-gray-500">₹599.00</p>
                    {/* <p className="mt-1">S, M, L, XL, XXL</p> */}
                  </h2>
                </div>
              </Link>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default Mugs;
